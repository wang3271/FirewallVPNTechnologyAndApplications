...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 | LISTEN| 	监听阶段|
| `client.py` 已连接，数据尚未发送完毕 |ESTABLISHED | 连接建立|
| `server.py` 执行 `close()` 之后 | TIME-WAIT| 连接关闭阶段|
...
| :--- | :------- | :------- |
| 程序是否自动退出 | 	是| 否|
| `ss` 中连接的 State | TIME-WAIT / CLOSE-WAIT|	ESTABLISHED |
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：会报 ConnectionRefusedError（连接被拒绝） 错误。此时客户端套接字存在，但服务器端没有创建监听套接字，也没有建立连接管道，因此无法通信。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server 套接字只负责监听连接请求，不能用于数据传输；accept () 返回的 conn 套接字专门用于和当前客户端进行数据收发。分开两个套接字可以让服务器同时监听新连接 + 和多个客户端通信。


3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：端口号是网络层面的标识，用于让外部主机找到当前程序；描述符是操作系统内部的编号，用于进程管理打开的资源；端口号解决网络定位问题，描述符解决系统资源管理问题。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？
   > 答：没有手动指定客户端端口号；端口由操作系统自动分配（临时端口 / 动态端口）；在 ss 命令输出中可以看到。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：因为 HTTP/1.0 默认使用短连接，服务器发送完响应后会主动关闭连接；客户端 recv() 会收到长度为 0 的数据，循环自动退出。


6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：HTTP/1.1 默认使用长连接（keep-alive），响应完成后不会立即断开；而 HTTP/1.0 默认短连接，响应后立即关闭。
